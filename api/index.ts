...
async function getUserByEmail(email: string) {
  const rows = await sql`
    SELECT id, email, password_hash, profile_name, exam_date, created_at
...
async function getUserStats(userId: number) {
  let rows = await sql`
    SELECT user_id, mastered_count, active_streak, last_quiz_score, total_time_studied, study_points, updated_at
    FROM user_stats WHERE user_id = ${userId}
  `;
  if (rows.length === 0) {
    await sql`
      INSERT INTO user_stats (user_id, mastered_count, active_streak, last_quiz_score, total_time_studied, study_points, updated_at)
      VALUES (${userId}, 64, 5, 8, 140, 340, NOW())
    `;
    rows = await sql`
      SELECT user_id, mastered_count, active_streak, last_quiz_score, total_time_studied, study_points, updated_at
      FROM user_stats WHERE user_id = ${userId}
...
async function updateUserStats(userId: number, updates: Record<string, any>) {
  if (Object.keys(updates).length === 0) return getUserStats(userId);

  if (updates.mastered_count !== undefined) {
    await sql`
      UPDATE user_stats SET mastered_count = ${updates.mastered_count}, updated_at = NOW() WHERE user_id = ${userId}
    `;
  }
  if (updates.active_streak !== undefined) {
    await sql`
      UPDATE user_stats SET active_streak = ${updates.active_streak}, updated_at = NOW() WHERE user_id = ${userId}
    `;
  }
  if (updates.last_quiz_score !== undefined) {
    await sql`
      UPDATE user_stats SET last_quiz_score = ${updates.last_quiz_score}, updated_at = NOW() WHERE user_id = ${userId}
    `;
  }
  if (updates.total_time_studied !== undefined) {
    await sql`
      UPDATE user_stats SET total_time_studied = ${updates.total_time_studied}, updated_at = NOW() WHERE user_id = ${userId}
    `;
  }
  if (updates.study_points !== undefined) {
    await sql`
      UPDATE user_stats SET study_points = ${updates.study_points}, updated_at = NOW() WHERE user_id = ${userId}
    `;
  }
...
async function getStudySessions(userId: number) {
  return await sql`
    SELECT id, user_id, date, module, score, total_questions, type
    FROM study_sessions
...
async function createStudySession(userId: number, session: any) {
...
async function setFlashcardMastery(userId: number, flashcardId: number, isMastered: boolean) {
  await sql`
    INSERT INTO flashcard_mastery (user_id, flashcard_id, is_mastered)
    VALUES (${userId}, ${flashcardId}, ${isMastered})
    ON CONFLICT (user_id, flashcard_id) DO UPDATE SET is_mastered = EXCLUDED.is_mastered
  `;
  return { user_id: userId, flashcard_id: flashcardId, is_mastered: isMastered };
}

async function resetFlashcardMasteries(userId: number) {
  await sql`
    DELETE FROM flashcard_mastery WHERE user_id = ${userId}
  `;
}
...
const getAiClient = () => {
  ...
  return new GoogleGenAI({
    ...
    httpOptions: {
      headers: {
        "User-Agent": "Enyi-Path-Serverless",
      },
    },
  });
};
...
async function retrieveKnowledgeChunks(query: string) {
  ...
  return topMatches.length > 0 ? topMatches.slice(0, 2) : [scored[0]];
}
...
app.post("/api/auth/register", async (req: any, res: any) => {
  try {
    ...
    const user = await createUser(email, passwordHash, profile_name, exam_date);
...
app.post("/api/user/stats", authenticate, async (req: any, res: any) => {
  try {
    ...
    const stats = await updateUserStats(req.userId, dbUpdates);
    const user = await getUserById(req.userId);
    return res.json({
      masteredQuestionsCount: stats.mastered_count,
      activeStreak: stats.active_streak,
...
app.get("/api/user/sessions", authenticate, async (req: any, res: any) => {
  try {
    const list = await getStudySessions(req.userId);
...
app.post("/api/user/sessions", authenticate, async (req: any, res: any) => {
  try {
    ...
    const saved = await createStudySession(req.userId, {
      id: id || "ses_" + Date.now(),
      date: date || "Just Now",
      module,
      score,
...
app.post("/api/user/sessions/clear", authenticate, async (req: any, res: any) => {
  try {
    await clearStudySessions(req.userId);
...
app.get("/api/user/flashcards", authenticate, async (req: any, res: any) => {
  try {
    const masteries = await getFlashcardMasteries(req.userId);
...
app.post("/api/user/flashcards/:id", authenticate, async (req: any, res: any) => {
  try {
    ...
    const row = await setFlashcardMastery(req.userId, cardId, isMastered);
    return res.json({
...
app.post("/api/user/flashcards/reset", authenticate, async (req: any, res: any) => {
  try {
    await resetFlashcardMasteries(req.userId);
  ...
  try {
    ...
    if (activeKeyType === null) {
      if (mode === "interview") {
        ...
        if (userMsgCount <= 1) {
          const reply = `AI service is unavailable right now. Running the mock interview in offline mode. Let's begin!\n\nOfficer Enyi: "Can you state your full legal name and define what 'naturalization' means?"`;
          return res.json({ content: reply, sources: [] });
        }

        const lastUserAnsIndex = userMsgCount - 2;
...
- Ask about allegiance, willingness to take the full Oath, willingness to bear arms, or perform noncombatant services.`;
...
- Provide a full realistic mix: Start with brief greeting/N-400 biographical data, transition to 5 civics questions, ask travel/employment history, and finish with Yes/No moral questions.`;
      }
...
app.post("/api/evaluate", authenticate, async (req: any, res: any) => {
  try {
    ...
    if (activeKeyType === null) {
      ...
      const outcome = isPartial ? "PARTIAL_COMPLETED" : (readiness >= 70 ? "PASS" : "FAIL");
